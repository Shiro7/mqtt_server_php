...
if ($mqtt->connect()) 
{
  if($msg2)
  {    
    $status1 ='{"s":"'.$msg2.'","a":'.$from_hour.',"b":'.$from_min.',"h":'.$from_hour.',"i":'.$from_min.',"m":'.$to_hour.',"n":'.$to_min.',"o":'.$to_sec.'}';
    ...
    if($data != "{\"status\":\"ON\"}")
    {
      ...
      if ($x == "{\"status\":\"Order Saved\"}") 
      {
        ...
        global $connection;
        $timer = '{"timer":"'.$from_hour.':'.$from_min.' & '.$to_sec.'s}';
        $sql = "UPDATE manage SET timer= '$timer' WHERE id=1";
      ...
      { 
        ...
        $timer = "{\"timer\":\"NO Order Recorded\"}";
        $sql = "UPDATE manage SET value= '$error' ,timer= '$timer' WHERE id=1";
        $connection->query($sql);
        header("Location:application.php");
      }
      else
